...
def create_phrase(text: str, n: int, vocab: utils.Vocablury):
    ...
    for sentence in sentences:
        ...
        if len(words) >= 1:
            s = len(words)
            for i in range(s):
                samples.append((n-i)*[vocab.index(vocab.sos)] +
                               words[max(i-n, 0):min(i+n+1, s)] +
                               (i+n+1-s)*[vocab.index(vocab.eos)])
    return samples


def create_sample(phrase: tf.Tensor):
    n = phrase.shape[0]//2
    x = tf.concat([phrase[:n], phrase[-n:]], axis=0)
    x = tf.reshape(x, [2*n])
    y = phrase[n]
    return x, y
...
class EmbeddingModel(Model):
    def __init__(self, vocab_size: int, embedding_size: int = 300):
        ...
        self.encoder = tf.keras.layers.GlobalAveragePooling1D()
        self.decoder = tf.keras.layers.Dense(units=vocab_size)

    def call(self, x, training=False):
        n_embedding = self.word_embedding(x, training=training)
        context = self.encoder(n_embedding)
...
class WordEmbedder(object):
    ...
    def save(self):
        ...
        with open("results/meta.tsv", mode="w") as meta_file, open("results/vecs.tsv", mode="w") as vec_file:
